...
// ==================== МАГИЧЕСКИЕ ЧИСЛА ДЛЯ EEPROM ====================
// Основные магические числа
const uint16_t EEPROM_MAGIC_V32 = 0xA1B2;  // Для AUX3 (V32) - ДОБАВЛЕНО
const uint16_t EEPROM_MAGIC_V34 = 0xA9B0;  // Для AUX3 (старый адрес)
const uint16_t EEPROM_MAGIC_V36 = 0xB1C2;  // Для AUX4
const uint16_t EEPROM_MAGIC_V37 = 0xB3C4;  // Для AUX5
const uint16_t EEPROM_MAGIC_V44 = 0xA1B2;  // Для UV уставки
const uint16_t EEPROM_MAGIC_V50 = 0xA3B4;  // Для таймера LOW
const uint16_t EEPROM_MAGIC_V51 = 0xA5B6;  // Для таймера HIGH
const uint16_t EEPROM_MAGIC_V52 = 0xA7B8;  // Для таймера ENABLED

// Обратите внимание: EEPROM_MAGIC_V32 и EEPROM_MAGIC_V44 имеют одинаковое значение 0xA1B2
// Это нормально, так как они хранятся в разных адресах
...
const unsigned long AUTO_SAVE_INTERVAL = 10000;  // 10 секунд между автосохранениями
const int EEPROM_MAX_WRITES = 100000;  // Максимальное число записей EEPROM
...
// Добавляем адреса для разрешений нагрева
#define EEPROM_V7_VALUE 130      // heatingPermission1
#define EEPROM_V7_MAGIC 134
#define EEPROM_V17_VALUE 136     // heatingPermission2
#define EEPROM_V17_MAGIC 140
...
const uint16_t EEPROM_MAGIC_V7 = 0xC1D2;
